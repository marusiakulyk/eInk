...
static void stack_event_handler(uint32_t event, void* eventParam)
{
    switch(event)
    {
        ...
        case CY_BLE_EVT_GATTS_WRITE_REQ:
        {
        	...
        	if(CY_BLE_LINEDATA_DATA_CHAR_HANDLE == writeReqParameter->handleValPair.attrHandle)
        	{
        		uint16_t len = writeReqParameter->handleValPair.value.len;
        		printf("[INFO] : Someone write next message\r\n HEX: ");
        		for(uint16_t i = 0; i < len; i++) {
        			printf("%x", writeReqParameter->handleValPair.value.val[i]);

        		}
        		printf("\r\n ASCII: ");
        		for(uint16_t i = 0; i < len; i++) {
        			printf("%c", writeReqParameter->handleValPair.value.val[i]);
        		}
